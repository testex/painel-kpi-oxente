...
const clientesService = new clientesService_1.ClientesService();
...
router.get('/', async (req, res) => {
    ...
    try {
        const filtros = {
            ...
            tipoPessoa: req.query.tipoPessoa,
...
router.get('/analytics/geral', async (req, res) => {
...
router.get('/analytics/rfm', async (req, res) => {
...
router.get('/estados/lista', async (req, res) => {
    ...
    try {
        ...
        const estadosUnicos = [...new Set(clientes
...
router.get('/cidades/lista', async (req, res) => {
    ...
    try {
        ...
        const cidadesUnicas = [...new Set(clientes
                .flatMap(cliente => cliente.enderecos
                .filter(endereco => endereco.estado === estado)
                .map(endereco => endereco.cidade))
                .filter(cidade => cidade && cidade.trim() !== ''))].sort();
